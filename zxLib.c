...
STIM_T stims = {.stim1Length = 1000u, .stim2Length = 1000u, .respCueLength = 500u,
    ...
    .currentDistractor = 7u, .distractorJudgingPair = 8u, .falseAlarmPenaltyRepeat = 0};
...
static int waterNResult(int firstOdor, int secondOdor, float waterPeroid, int id) {
    int rtn = 0;
    int rewardWindow = (taskType == ODPA_RD_SHAPING_A_CATCH_LASER_TASK
            || taskType == ODPA_RD_SHAPING_B_CATCH_LASER_TASK
            || taskType == ODPA_RD_CATCH_LASER_TASK) ? 1000 : 500;
    ...
    switch (taskType) {
        ...
        default:
            ...
            ///////////Detect/////////////////
            for (timerCounterI = 0; timerCounterI < rewardWindow && !lick.flag; lick.flag = lick.current);
            ...
            for (iterOf4 = 0; iterOf4 < 4 && currentMiss < missLimit; iterOf4++) {
                ...
                switch (taskType) {
                    ...
                    case ODPA_RD_CATCH_LASER_TASK:
                        if (!stims.falseAlarmPenaltyRepeat) {
                            firstOdor = (index == 0 || index == 2) ? stim1 : (stim1 + 1);
                            secondOdor = (index == 1 || index == 2) ? stim2 : (stim2 + 1);
                        }
                        if (currentTrial > 15) {
...
static void stim(int place, int stim, int type) {
    switch (stim) {
        ...
        case 20://FAKE ODAP 
            switch (place) {
                case 1:
                    assertLaser(type, atFirstOdorBeginning);
                    break;
                case 2:
                    assertLaser(type, atSecondOdorBeginning);
                    break;
                case 3:
                    assertLaser(type, atResponseCueBeginning);
                    break;
            }
            ...
            switch (place) {
                case 1:
                    lcdWriteChar('1', 4, 1);
                    waitTimerJ(stims.stim1Length);
                    break;
                case 2:
                    lcdWriteChar('2', 4, 1);
                    waitTimerJ(stims.stim2Length);
                    break;
                case 3:
                    lcdWriteChar('3', 4, 1);
                    waitTimerJ(stims.respCueLength);
                    break;
            }
            Valve_OFF(stim);
            Out2 = 0;
            ...
            switch (place) {
                case 1:
                    assertLaser(type, atFirstOdorEnd);
                    lcdWriteChar('d', 4, 1);
                    waitTimerJ(1000u - stims.stim1Length);
                    break;
                case 2:
                    assertLaser(type, atSecondOdorEnd);
                    lcdWriteChar('D', 4, 1);
                    waitTimerJ(1000u - stims.stim2Length);
                    break;
                case 3:
                    assertLaser(type, atResponseCueEnd);
                    break;
            }
            break;
            //        case 20:
...
unsigned int responseDelayLicked(unsigned int lengthInMilliSec) {
    unsigned int targetTime = timerCounterJ + lengthInMilliSec;
    lick.flag = 0;
    while (timerCounterJ < targetTime) {
        if (lick.current) {
            lick.flag = 1;
        }
    }
    waitTimerJ(lengthInMilliSec);
    return lick.flag;
}
...
static void zxLaserTrial(int type, int firstOdor, STIM_T odors, _delayT interOdorDelay, int secondOdor, float waterPeroid, unsigned int ITI) {
    ...
    unsigned int delayLick = responseDelayLicked(1500u);
    int resultRtn = 0;
    if (delayLick) {
        protectedSerialSend(SpAbortTrial, 1);
        lcdWriteChar('A', 4, 1);
        waitTimerJ(500u);
    } else {
        assertLaser(type, atRewardBeginning);
        lcdWriteChar('R', 4, 1);
        stim(3, stims.respCueValve, type);
        ...
        resultRtn = waterNResult(firstOdor, secondOdor, waterPeroid, id);
    }
    waitTimerJ(1050u); //water time sync
    // Total Trials
    ...
    if (resultRtn == SpFalseAlarm) {
        if (punishFalseAlarm == FALSE_ALARM_PENALTY_TIMEOUT) {
            ITI *= 3u;
        } else if (punishFalseAlarm == FALSE_ALARM_PENALTY_REPEAT) {
            stims.falseAlarmPenaltyRepeat = 1;
        }
    } else {
        stims.falseAlarmPenaltyRepeat = 0;
    }
...
void callFunction(int n) {
    ...
    switch (n) {
        ...
        case 4377:
        {
            ...
            highLevelShuffleLength = 20;
            laserSessionType = LASER_SESS_UNDEFINED;
            taskType = ODPA_RD_CATCH_LASER_TASK;
            splash("Sample Odor", "");
            int sampleType = setType();
            splash("Test Odor", "");
            int testType = setType();
            zxLaserSessions(sampleType, testType, laserOff, 5u, 8u, 20u, 0.05, 20, setSessionNum());
            break;
        }
        ...
        case 4378:
        {
            ...
            splash("Test Odor", "");
            int testType = setType();
            punishFalseAlarm = FALSE_ALARM_PENALTY_TIMEOUT;
            zxLaserSessions(sampleType, testType, laserOff, 5u, 8u, 20u, 0.05, 20, setSessionNum());
            break;
        }
        case 4379:
        {
            splash("ODPA R_D", "REPEAT");
            highLevelShuffleLength = 20;
            laserSessionType = LASER_SESS_UNDEFINED;
            taskType = ODPA_RD_CATCH_LASER_TASK;
            splash("Sample Odor", "");
            ...
            splash("Test Odor", "");
            int testType = setType();
            punishFalseAlarm = FALSE_ALARM_PENALTY_REPEAT;
            zxLaserSessions(sampleType, testType, laserOff, 5u, 8u, 20u, 0.05, 20, setSessionNum());
            break;
        }
