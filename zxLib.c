...
static void waitJ(unsigned int dT) {
    timeSum += dT;
...
static void zxLaserSessions(int odorType, int laserType, _delayT delay, int ITI, int trialsPerSession, float WaterLen, int missLimit, int totalSession, float delay_before_reward) {
    //    wait_ms(1000);
    int currentTrial = 0;
...
static void zxLaserTrial(int type, int firstOdor, STIM_T odors, _delayT interOdorDelay, int secondOdor, float waterPeroid, int ITI, float delay_before_reward, int laserOnTrial) {
    ...
    if (taskType == GONOGO_LR_TASK || taskType == GONOGO_TASK || taskType == GONOGO_LR_TEACH) {
...
void callFunction(int n) {
    ...
    switch (n) {
        ...
        case 4302:
        {
            ...
            zxLaserSessions(setType(), laserDuringDelayChR2, delay, delay * 2, 16, 0.05, 20, setSessionNum(), 1.0);
            break;
        }
        case 4303:
        ...
        case 4415:
        {
            ...
            zxLaserSessions(setType(), laserDuringDelayChR2, delay, delay * 2, 20, 0.05, 50, setSessionNum(), 1.0);
            break;
        }
